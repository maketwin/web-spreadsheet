...
复制/剪切除了写系统剪贴板，还会在应用内建立一个**剪贴板会话**（内置 UI 的行为；`ClipboardService` 本身保持无状态）：

- 复制/剪切后，源区域显示 Excel 风格的**蚂蚁线**（流动虚线框），并在与选中范围重合时**替换选区实线边框与填充柄**——与 Excel 视觉一致，填充柄与边框拖拽此时不可用；
- 粘贴优先使用会话快照；没有会话时读系统剪贴板（跨应用粘贴）；
- **复制**会话可重复粘贴；**剪切**是移动语义——粘贴落地时才清空源区域，且只能粘贴一次；
- 取消会话的方式：`Esc`，或开始编辑复制范围内的任意单元格；
- 再次复制/剪切会以新范围重建会话。
