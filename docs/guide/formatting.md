...
本章覆盖三种「让单元格更好看 / 更受控」的机制：数字格式、条件格式（数据条 / 色阶 / 单元格值 / 公式）、数据验证。它们都挂在样式/规则系统上，通过命令修改、可撤销。
...
::: warning 部分支持
已支持 `[Red]`/`[Blue]` 等命名颜色段，以及 `[>100]`/`[<=0]` 条件段。`[$¥-804]` 等 locale 货币选择器仍跳过。格式串无法解析时单元格回退显示原文。
:::

### 工具栏：增加/减少小数位数

工具栏「增加小数位数 / 减少小数位数」按钮（`.0→.00` / `.00→.0`）对齐 Excel 数字组行为，由 `adjustDecimalPlaces`（`src/format/decimalPlaces.ts`）推导新格式串：

- 常规格式：增加 → `0.0`；减少 → 无操作；
- 内置 `number` / `percent`：先落到等价自定义串（`#,##0.00` / `0.00%`）再逐位调整；
- 自定义格式：对正数段的小数位增/减一个 `0`，减到 0 位时连同小数点去掉；无小数点时往整数位后插入 `.0`（`%` 之前）；
- `currency` / `date` / `time` / `scientific`：不可调，按钮无效果（与 Excel 在不可调格式上的表现一致）；
- 小数位上限 30。
