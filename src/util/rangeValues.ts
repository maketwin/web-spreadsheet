...
const clearPatch = (): CellPatch => ({ text: '', formula: undefined, value: undefined, styleId: undefined, type: undefined, richText: undefined, hyperlink: undefined });
export function clearRange(store: Store, cmdManager: CommandManager | undefined, range: RangeAddress): void { const values = matrix(range, clearPatch); executeRange(store, cmdManager, range.r1, range.c1, values); }
export function clearRangeCmd(range: RangeAddress): Command { const values = matrix(range, clearPatch); return new SetRangeValues({ r1: range.r1, c1: range.c1, r2: range.r2, c2: range.c2, values }); }
