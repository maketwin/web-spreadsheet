...
export class SetRowsHiddenCommand extends Command<SetRowsHiddenArgs> {
  ...
  public execute(store: Store): void {
    ...
    for (let r = this.args.r1; r <= this.args.r2; r += 1) {
      ...
      metas.push(meta);
      store.setRow(r, { ...meta, hide: this.args.hidden });
    }
    this.oldMetas = metas;
  }

  public getUndo(): Command {
    return new RestoreRowsMeta({ r1: this.args.r1, metas: this.oldMetas.slice() });
  }
...
class RestoreRowsMeta extends Command<RestoreRowsMetaArgs> {
  public execute(store: Store): void {
    this.args.metas.forEach((meta, i) => store.setRow(this.args.r1 + i, meta));
  }
...
export class SetColsHiddenCommand extends Command<SetColsHiddenArgs> {
  ...
  public execute(store: Store): void {
    ...
    for (let c = this.args.c1; c <= this.args.c2; c += 1) {
      ...
      metas.push(meta);
      store.setCol(c, { ...meta, hide: this.args.hidden });
    }
    this.oldMetas = metas;
  }

  public getUndo(): Command {
    return new RestoreColsMeta({ c1: this.args.c1, metas: this.oldMetas.slice() });
  }
...
class RestoreColsMeta extends Command<RestoreColsMetaArgs> {
  public execute(store: Store): void {
    this.args.metas.forEach((meta, i) => store.setCol(this.args.c1 + i, meta));
  }
